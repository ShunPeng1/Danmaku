...
namespace _Scripts.CoreGame.InteractionSystems.Roles
{
    public class PartnerDanmakuRole : IDanmakuRole
    {
        ...
        {
            var bossPlayers = _danmakuPlayerSubsystem.PlayerRoles.FindAll(player => player.Role.HasRole(DanmakuRoleEnum.StageBoss) || player.Role.HasRole(DanmakuRoleEnum.ExtraBoss));
